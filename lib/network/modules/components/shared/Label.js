...
class Label {
  ...
  */
  ...
  _drawText(ctx, selected, hover, x, y, baseline = 'middle') {
    ...
    let redFlagColor        =  '#ff6802';
    let networkBg           =  'rgba(26,26,28,0.75)';

    let nodeLabelBoxStroke  = 'rgba(255,255,255,0.15)';
    let nodeLabelBoxFill    = 'rgba(0,0,0,0.3)';

    let nodeFlagBoxStroke   = 'rgba(255,255,255,0.15)';
    let nodeFlagBoxFill     = 'rgba(0,0,0,0.5)';
    let nodeFlagBoxText     = redFlagColor;

    let edgeLabelBoxStroke  = 'rgba(255,255,255,0.15)';
    let edgeLabelBoxFill    = networkBg;
    let edgeLabelText       = redFlagColor;
    ...
    for (let i = 0; i < this.lineCount; i++) {
      if (this.lines[i] && this.lines[i].blocks) {
        ...
        for (let j = 0; j < this.lines[i].blocks.length; j++) {
          ...
          // Edge Label
          if (this.isEdgeLabel) {
              block.color = edgeLabelText;
              fontColor = edgeLabelText;
              
              ctx.strokeStyle = edgeLabelBoxStroke;
              ctx.fillStyle = edgeLabelBoxFill;
              this.roundRect(ctx, x - 3 , yLine - this.lines[i].height/2 - 1  , block.width + 6, this.lines[i].height + 3, 4, true);

          // Node Label
          } else {
            // node name label border
            if (i == 0) {
              block.vadjust = this.lineCount > 1 ? 10 : 4;
              ctx.strokeStyle = nodeLabelBoxStroke;
              ctx.fillStyle = nodeLabelBoxFill;
              
              //  roundRect(ctx, x, y, width, height, radius, fill, stroke)
              //this.roundRect(ctx, x + width - 3, yLine + block.vadjust - 3, block.width + 6, this.lines[i].height + 6, 10, true);
              this.roundRect(ctx, x + width - 6, yLine + block.vadjust - 2, block.width + 12, this.lines[i].height + 3, 7.5, true);
            }
            
            // node red flags border
            if (i == 1) {
              block.vadjust = 10;
              block.color = nodeFlagBoxText;
              fontColor = nodeFlagBoxText;
              fontSize = 5;
                            
              ctx.strokeStyle = nodeFlagBoxStroke;
              ctx.fillStyle = nodeFlagBoxFill;
              this.roundRect(ctx, x + width - 3 , yLine + block.vadjust - 1 , block.width + 6, this.lines[i].height + 1, 4, true);
            }
          }
  ...
  _processLabel(ctx, selected, hover) {
    ...
    if (this.elementOptions.label !== undefined) {
      ...
      if (this.elementOptions.flags !== undefined) {  
        if(Object.keys(this.elementOptions.flags).length > 0) {
          flags += '\n'; 
          for(let i = 0; i < Object.keys(this.elementOptions.flags).length; i++){
             flags += '⚑';
          }        
        } 
      } 
      let labelText = this.elementOptions.label + flags;
      
      let nlLines = String(labelText).split('\n');
