...
class ClusterEngine {
  ...
  */
  _createClusterEdges (childNodesObj, childEdgesObj, clusterNodeProperties, clusterEdgeProperties) {
      ...
      for (let j = 0; j < childNode.edges.length; j++) {
        ...
        if (this.clusteredEdges[edge.id] === undefined) {
          ...
          if (childNodesObj[otherNodeId] === undefined) {
                let hidden = false;
                let value = edge.options.value;
                for(let  _j1=0; _j1 < createEdges.length; _j1++) {
                  let createEdge = createEdges[_j1];
                  console.log(`toId ${toId} fromId ${toId} createEdge -> from ${createEdge.fromId} to ${createEdge.toId}`);
                  if((createEdge.fromId === fromId && createEdge.toId === toId)
                    || (createEdge.toId === fromId && createEdge.fromId === toId)
                  ) {
    ...
    let lastEdge = createEdges.length - 1;
    ...
    for (let j = 0; j < createEdges.length; j++) {
      ...
      clusterEdgeProperties = {hidden: createEdges[j].hidden, value: createEdges[j].value};
